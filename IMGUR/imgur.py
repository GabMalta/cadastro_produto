...
import requests
from IMGUR.requests_policy.http import http
from IMGUR.core import settings
...
class Imgur():
  ...
  def get_refresh_token(self):
    ...
    path_refresh_token = os.path.join(settings.BASEDIR,r'tokens_imgur/refresh_token.txt')
  ...
  def get_access_token(self):
    ...
    with open(os.path.join(settings.BASEDIR,r'tokens_imgur/access_token.txt'), 'r') as at:
  ...
  def set_access_token(self, access_token):
    ...
    with open(os.path.join(settings.BASEDIR,r'tokens_imgur/access_token.txt'), 'w') as at:
      access_token = at.write(access_token)

  def set_refresh_token(self, refresh_token):
    
    with open(os.path.join(settings.BASEDIR,r'tokens_imgur/refresh_token.txt'), 'w') as rt:
  ...
  def create_album(self, title, images:list=[]):
    ...
    print(f'Album criado com sucesso!')
  ...
  def set_rates_limits(self, response:Response) -> None:
    ...
    with open(os.path.join(settings.BASEDIR,'./rate_limits_api/rate_limit.txt'), 'w') as res:
      res.write(client_limit)
    with open(os.path.join(settings.BASEDIR,'./rate_limits_api/rate_client_remaining.txt'), 'w') as res:
      res.write(client_remaining)
    with open(os.path.join(settings.BASEDIR,'./rate_limits_api/rate_user_limit.txt'), 'w') as res:
      res.write(user_limit)
    with open(os.path.join(settings.BASEDIR,'./rate_limits_api/rate_user_remaining.txt'), 'w') as res:
      res.write(user_remaining)
    with open(os.path.join(settings.BASEDIR,'./rate_limits_api/rate_user_reset.txt'), 'w') as res:
