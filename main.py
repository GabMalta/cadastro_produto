...
from apps.bling_api.schemas.fornecedor import Contato, Fornecedor
...
from apps.bling_api.schemas.produto import ProdutoSchema
import settings
...
def main():
    ...
    for i, info in enumerate(form_response["sizes_and_dimensions"]):
        ...
        produto.camposCustomizados[3].valor = f"{info['size']}m x {form_response['width']}m"
        produto.camposCustomizados[7].valor = form_response["composition"]
        produto.camposCustomizados[8].valor = form_response["width"]
        produto.camposCustomizados[11].valor = codigo
