...
class BlingApi:
    ...
    def get_access_token(self):
        with open(
            os.path.join(settings.PATH_SRC, "tokens/access_token.txt"), "r"
        ) as token:
            self.access_token = token.read()

    def get_refresh_token(self):
        with open(
            os.path.join(settings.PATH_SRC, "tokens/refresh_token.txt"), "r"
        ) as token:
            self.refresh_token = token.read()

    def set_access_token(self, access_token: str):
        with open(
            os.path.join(settings.PATH_SRC, "tokens/access_token.txt"), "w"
        ) as token:
    ...
    def set_refresh_token(self, refresh_token: str):
        with open(
            os.path.join(settings.PATH_SRC, "tokens/refresh_token.txt"), "w"
        ) as token:
                ...
                try:
                    ...
                    match type_error:
                        ...
                        case "Invalid refresh token":
                            msg = (
                                "Refresh token inválido ou expirado, gere novos tokens"
                            )
    ...
    def get_products(self, params:dict=None):
        ...
        link = f"https://www.bling.com.br/Api/v3/produtos?{params}"
        headers = {
            "Authorization": f"Bearer {self.access_token}",
            "Content-Type": "application/json",
        }
        ...
        try:
            response = http.get(link, headers=headers, params=params).json()
            return response
        except requests.exceptions.HTTPError as err:
    ...
    def delete_products(self, params:list):
        ...
        try:
            response = http.delete(link, headers=headers, params=params, timeout=(10, 60)).json()
            return response
        except requests.exceptions.HTTPError as err:
            print(err.response.json())
    ...
    def create_product(self, product: ProdutoSchema):
        ...
        try:
            response = http.post(link, headers=headers, data=product_json)
            # print(f"Produto {product_json} cadastrado com sucesso. {response.status_code}")
    ...
    def create_link_with_store(self, vinculo: ProdutoLojaSchema):
        ...
        try:
            response = http.post(link, headers=headers, data=payload)
            response = response.json()
